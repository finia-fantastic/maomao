  ...
  return defineConfig({
    presets: [
      ...
      presetIcons({
        ...
        collections: {
          // NOTICE:
          // presetIcons' built-in Node auto-loader fails to resolve `@iconify-json/*`
          // collections in this monorepo's pnpm layout, producing zero icon CSS for
          // `i-solar:*`, `i-ph:*`, etc. (verified: bare `presetIcons()` -> 0 css;
          // `createExternalPackageIconLoader('@iconify-json/solar')` -> icon css). The
          // external-package loader resolves the collection packages deterministically,
          // so every installed `@iconify-json` collection is registered explicitly here.
          // Requires those packages to be resolvable at the config root — see the
          // `public-hoist-pattern` entries in `.npmrc`.
          ...Object.assign(
            {},
            ...['carbon', 'eos-icons', 'lucide', 'mingcute', 'ph', 'simple-icons', 'solar', 'svg-spinners', 'vscode-icons']
              .map(name => createExternalPackageIconLoader(`@iconify-json/${name}`)),
          ),
