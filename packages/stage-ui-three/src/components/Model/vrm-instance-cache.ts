...
export interface ManagedVrmInstance {
  ...
  /**
   * The idle-loop clip bound to `mixer`. Carried with the instance so that
   * after a cache reuse (remount / model-switch back), gesture playback can
   * still re-acquire the idle action and cross-fade back to it.
   */
