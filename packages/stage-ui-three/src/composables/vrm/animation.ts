...
/** Options for playing a one-off body-gesture clip over the idle loop. */
export interface VrmPlayAnimationOptions {
  /**
   * Loop the clip instead of playing it once and reverting to idle.
   * @default false
   */
  loop?: boolean
  /**
   * Cross-fade duration in seconds, used both when fading the gesture in and,
   * for non-looping gestures, when fading back out to the idle loop.
   * @default 0.4
   */
...
/**
 * Cross-fades a mixer from its current action to `to`, starting `to` from a
 * clean state so repeated plays always begin at frame zero.
 *
 * `from` is optional because the first gesture after model load only has the
 * idle action to fade from; callers pass `undefined` when there is nothing to
 * blend out of and `to` simply fades in.
 *
 * Side effect: mutates weights/time on the shared AnimationMixer's actions.
 */
export function crossFadeToAction(from: AnimationAction | undefined, to: AnimationAction, duration: number) {
  to.enabled = true
  to.setEffectiveTimeScale(1)
  to.setEffectiveWeight(1)
  to.reset()
  to.play()
  // warp=true keeps the blend perceptually smooth when the two clips run at
  // different effective speeds. Skip crossFade when re-triggering the same
  // action, since crossFadeFrom against itself would zero out its own weight.
  if (from && from !== to)
