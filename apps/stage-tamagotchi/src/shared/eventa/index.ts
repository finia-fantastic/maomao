...
/** A snapshot of the user's vocabulary-learning progress, read from the shared `words.db` SQLite file written by the external Python vocab app. */
export interface VocabStats {
  /** Total number of words in the database. */
  total: number
  /** Words added today (`add_date = DATE('now')`). */
  today: number
  /** Words with `status = 'learning'`. */
  learning: number
  /** Words with `status = 'reviewing'`. */
  reviewing: number
  /** Words with `status = 'mastered'`. */
  mastered: number
  /** Words whose `next_review` date is today or earlier (due to review now). */
  dueForReview: number
  /** Most recently added words, newest first (capped list for the LLM to reference). */
  recent: { english: string, chinese: string, status: string, masteryLevel: number }[]
  /** Set when the DB could not be read (missing file, locked, etc.); numeric fields are 0 and `recent` empty. */
  error?: string
}
export const electronVocabStats = defineInvokeEventa<VocabStats>('eventa:invoke:electron:vocab:stats')
export const electronOpenVocabApp = defineInvokeEventa<{ ok: boolean, error?: string }>('eventa:invoke:electron:vocab:open-app')
